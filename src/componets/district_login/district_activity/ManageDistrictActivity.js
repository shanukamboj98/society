...
const ManageDistrictActivity = () => {
  ...
  // Fetch all activities from API - filtered by created_by (district admin) when available
  const fetchAllActivities = async () => {
    setIsLoading(true);
    try {
      let url = "https://mahadevaaya.com/ngoproject/ngoproject_backend/api/activity-items/";
      ...
      console.log("GET All District Activities API Response:", result);
      ...
      // Filter activities to show only those created by current district admin
      if (auth && auth.unique_id) {
        activitiesData = activitiesData.filter(
          activity => activity.created_by === auth.unique_id || activity.created_by == auth.unique_id
        );
      }
  ...
  const fetchActivityData = async (activityId) => {
    ...
    try {
      ...
      setFormData({
        ...
        allocated_district: activityData.allocated_district || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Submitting data for district activity:", formData.activity_name);
...
export default ManageDistrictActivity;